...
private extension RestaurantDetailViewController {
    ...
    func displayWeather() {
        Task {
            if let lat = selectedRestaurant?.lat, let long = selectedRestaurant?.long {
                let restLocation = CLLocation(latitude: lat, longitude: long)
                let restWeather = await weatherDataManager.fetchWeather(at: restLocation)
                ...
                let weatherImage = UIImage(systemName: restWeather.0, withConfiguration: combined)?.withTintColor(.white)
                let attachment = NSTextAttachment()
                attachment.image = weatherImage
                let imageString = NSMutableAttributedString(attachment: attachment)
                let textString = NSAttributedString(string: " " + restWeather.1)
